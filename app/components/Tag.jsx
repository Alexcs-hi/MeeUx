...
export default function Tag({ name, onDelete, setExcluded, isExcluded }) {
    ...
    return (
        <div className="cursor-pointer flex items-center gap-2 border border-gray-400/30 rounded-xl p-2 h-11">

            <button onClick={onDelete}>
                <h1 className={` cursor-pointer hover:bg-red-500/50 p-1 rounded ${isExcluded ? "line-through text-gray-400 " : ""}`}>{name}</h1>
            </button>
