...
export default function SearchBar() {
  ...
  function toggleExcluded(index) {
    setQueryList((prev) => prev.map((tag, i) => (i === index ? { ...tag, excluded: !tag.excluded } : tag)));
  }
  ...
  return (
    <div className='w-full flex flex-col gap-4   '>
      ...
      {queryList && queryList.length > 0 && <h1 className='text-gray-300 md:text-sm  text-xs  mt-2'>Selected Tags</h1>}
      ...
      {queryList && queryList?.length > 0 && (
        <div className='flex border-t pt-4 border-gray-400/30 gap-2 flex-wrap  max-h-30 lg:max-h-60 md:max-h-40  overflow-auto'>
          {queryList.map((q, index) => (
            <Tag
              isExcluded={q.excluded}
              setExcluded={() => toggleExcluded(index)}
      ...
      {queryList && queryList.length > 0 && (
        <button
          ...
          className='text-gray-400 md:text-md text-sm p-2 mt-2 border border-gray-200/20 rounded-xl hover:bg-gray-200/20 transition'>
          Clear All
        </button>
      )}
    </div>
