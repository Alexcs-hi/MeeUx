...
function SortBar() {
  ...
  const selectedScore = scoreOptions.find((opt) => opt.value == score)?.name || "Score";

  const selectedRating = ratingOptions.find((opt) => opt.value == rating)?.name || "Rating";

  const selectedUpload = uploadOptions.find((opt) => opt.value == upload)?.name || "Upload";
